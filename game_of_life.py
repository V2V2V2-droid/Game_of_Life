...
array2 = np.random.choice([0, 1], N*N, p=[0.1, 0.9]).reshape(N, N)
...
def neigboors_alive(grid):
    ...
    for (i, j), value in np.ndenumerate(grid):
        ...
        mesh = [(k[0], k[1]) for k in mesh if 0 <= k[0] < N and 0 <= k[1] < N]
...
if __name__ == "__main__":
    A = Gol(initial_state=array_test, rules=standard_rule, duration=50).run_game()
    fig = px.imshow(A, animation_frame=0,
                    binary_string=True, labels=dict(animation_frame="time steps in the life journey"))
    fig.show()
